...
void USBHS_Device_Endp_Init( void )
{
    ...
    USBHSD->UEP1_TX_DMA = (uint32_t)RNDIS_EP1_Buffer;
    USBHSD->UEP2_TX_DMA = (uint32_t)RNDIS_TX_Buffer;
    USBHSD->UEP3_RX_DMA = (uint32_t)RNDIS_RX_Buffer;
...
void USBHS_IRQHandler( void )
{
                ...
                switch( intst & ( USBHS_UIS_TOKEN_MASK | USBHS_UIS_ENDP_MASK ) )
                {
                    ...
                    case USBHS_UIS_TOKEN_OUT | DEF_UEP3:
                        ...
                        else
                        {
                            ...
                            USBHS_Endp3_RxLen += len;
                            USBHS_UsbRxFlag = 1;
                            USBHS_UsbRxLen = USBHS_Endp3_RxLen;
                            USBHS_Endp3_RxLen = 0;
                            /* Move USB DMA Address */
                            USBHSD->UEP3_RX_DMA = ( uint32_t )RNDIS_RX_Buffer;
                        ...
                        if ( intst & USBHS_UIS_TOG_OK )
                        {
                            ...
                            if ( ( USBHS_SetupReqType & USB_REQ_TYP_MASK ) != USB_REQ_TYP_STANDARD )
                            {
                                ...
                                if( USBHS_SetupReqLen != 0 )
                                {
                                    ...
                                    {
                                        memcpy( ( void *)pRNDIS_Message, USBHS_EP0_Buf, DEF_USBD_UEP0_SIZE );
                                        pRNDIS_Message += DEF_USBD_UEP0_SIZE;
                                        USBHSD->UEP0_RX_CTRL ^= USBHS_UEP_R_TOG_DATA1;
                                    }
                                    else
                                    {
                                        memcpy( ( void *)pRNDIS_Message, USBHS_EP0_Buf, USBHS_SetupReqLen );
                                        RNDIS_MSG_Recv( );
                                    }
                                }
                            }
                ...
                switch ( intst & ( USBHS_UIS_TOKEN_MASK | USBHS_UIS_ENDP_MASK ) )
                {
                    ...
                    case USBHS_UIS_TOKEN_IN | DEF_UEP0:
                        ...
                        if ( ( USBHS_SetupReqType & USB_REQ_TYP_MASK ) != USB_REQ_TYP_STANDARD )
                        {
                            ...
                            memcpy(USBHS_EP0_Buf, pRNDIS_Message, len);
                            USBHS_SetupReqLen -= len;
                            pRNDIS_Message += len;
    ...
    else if( intflag & USBHS_UIF_SETUP_ACT )
    {
        ...
        if ( ( USBHS_SetupReqType & USB_REQ_TYP_MASK ) != USB_REQ_TYP_STANDARD )
        {
            pRNDIS_Message = encapsulated_buffer;
            /* usb non-standard request processing */
            if( (USBHS_SetupReqType & USB_REQ_TYP_IN) && USBHS_SetupReqLen != 0)
            {
                /* RNDIS Message */
                ...
                case USB_GET_DESCRIPTOR:
                    switch( (uint8_t)(USBHS_SetupReqValue>>8) )
                    {
                        ...
                        case USB_DESCR_TYP_SPEED:
                            ...
                            {
                                /* High speed mode */
                                memcpy( &TAB_USB_HS_OSC_DESC[ 2 ], &MyCfgDescr_HS[ 2 ], DEF_USBD_CONFIG_FS_DESC_LEN - 2 );
