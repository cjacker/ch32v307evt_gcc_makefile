...
void USBHS_Device_Endp_Init( void )
{
    ...
    USBHSD->UEP2_TX_DMA = (uint32_t)USB_TxBuff;
    USBHSD->UEP3_RX_DMA = (uint32_t)USB_RxBuff;
...
void USBHS_IRQHandler( void )
{
                ...
                switch( intst & ( USBHS_UIS_TOKEN_MASK | USBHS_UIS_ENDP_MASK ) )
                {
                    ...
                    case USBHS_UIS_TOKEN_OUT | DEF_UEP3:
                        ...
                        else
                        {
                            ...
                            /* load length & addr */
                            USBHS_Endp3_RxLen += len;
                            USB_RxManage.Length = USBHS_Endp3_RxLen;
                            USBHS_Endp3_RxLen = 0;
                            /* Move USB DMA Address */
                            USBHSD->UEP3_RX_DMA = ( uint32_t )USB_RxBuff;
                            USB_RxManage.Flag = 1;
    ...
    else if( intflag & USBHS_UIF_SETUP_ACT )
    {
        ...
        if ( ( USBHS_SetupReqType & USB_REQ_TYP_MASK ) != USB_REQ_TYP_STANDARD )
        {
            ...
            switch( USBHS_SetupReqCode )
            {
                case DEF_NCM_GET_NTB_PARAMETERS:
                    /* 0x80: GetNtbParameters */
                    memcpy( USBHS_EP0_Buf, NTB_Parameter_Structure, USBHS_SetupReqLen );
                    break;
